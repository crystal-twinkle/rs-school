...
function chooseLevel(): void {
    allLevels.addEventListener('click', (event) => {
        const target: HTMLElement | EventTarget | null = <HTMLElement>event.target;
        if (target instanceof HTMLElement) {
            ...
            const number: number = firstTwoLitter.indexOf('&') !== -1 ? Number(firstTwoLitter.slice(0, 1)) : Number(firstTwoLitter);
